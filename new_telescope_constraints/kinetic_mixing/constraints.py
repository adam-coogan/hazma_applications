...
class Constraints(KineticMixing):
    ...
    def compute_telescope_constraints(
        ...
        new_telescopes: List[str], optional
            List of existing telescopes. If None, amego, adept, e-astrogam,
            gecco, grams, mast, and pangu are used.
        ...
        for name, diffuse in existing_telescopes.items():
            ...
            for i, mx in enumerate(tqdm.tqdm(mxs, desc="{:11}".format(name))):
                self.mx = mx
                self.mv = mvs[i]
    ...
    def compute_cmb_constraints(
        ...
        for i, mx in enumerate(tqdm.tqdm(mxs, desc="cmb")):
            self.mx = mx
            self.mv = mvs[i]
    ...
    def compute_pheno_constraints(
        ...
        if "babar" in experiments:
            ...
            for i, mx in enumerate(tqdm.tqdm(mxs, desc="babar")):
                self.mx = mx
                self.mv = mvs[i]
        ...
        if "lsnd" in experiments:
            ...
            for i, mx in enumerate(tqdm.tqdm(mxs, desc="lsnd ")):
                self.mx = mx
                self.mv = mvs[i]
        ...
        if "e137" in experiments:
            ...
            for i, mx in enumerate(tqdm.tqdm(mxs, desc="e137 ")):
                self.mx = mx
                self.mv = mvs[i]
